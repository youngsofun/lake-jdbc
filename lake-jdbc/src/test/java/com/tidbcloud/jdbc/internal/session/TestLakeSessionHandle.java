package com.tidbcloud.jdbc.internal.session;

import com.tidbcloud.jdbc.DriverInfo;
import com.tidbcloud.jdbc.internal.QueryResultFormat;
import com.tidbcloud.jdbc.internal.exception.LakePresignException;
import com.tidbcloud.jdbc.internal.exception.LakeQueryException;
...
@Test(timeOut = 10000)
public class TestLakeSessionHandle {
    @Test(groups = {"UNIT"})
    public void testUploadStreamUsesStageUploadEndpointWhenPresignDisabled() throws Exception {
        ...
        try {
            LakeSessionHandle handle = new LakeSessionHandle(
    ...
    public void testUploadStreamStageUploadUnauthorizedRaisesSQLExceptionWithStageUploadCause() throws Exception {
        ...
        Assert.assertTrue(exception.getCause() instanceof LakeStageUploadException, String.valueOf(exception.getCause()));
    ...
    @Test(groups = {"UNIT"})
    public void testExecuteStageUploadServiceUnavailableRaisesLakeStageUploadException() throws Exception {
        AtomicReference<Integer> attempts = new AtomicReference<>(0);
        OkHttpClient client = new OkHttpClient.Builder()
                .addInterceptor(userAgentInterceptor(DriverInfo.USER_AGENT_VALUE))
                .addInterceptor((Interceptor) chain -> {
        ...
        LakeStageUploadException exception = Assert.expectThrows(LakeStageUploadException.class, () ->
    ...
    @Test(groups = {"UNIT"})
    public void testExecuteStageUploadUnauthorizedRaisesLakeStageUploadException() throws Exception {
        AtomicReference<Integer> attempts = new AtomicReference<>(0);
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/upload_to_stage", exchange -> {
            try {
    ...
    @Test(groups = {"UNIT"})
    public void testExecuteStageUploadConfigurationErrorRaisesLakeStageUploadException() throws Exception {
        AtomicReference<Integer> attempts = new AtomicReference<>(0);
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/upload_to_stage", exchange -> {
            try {
    ...
    @Test(groups = {"UNIT"})
    public void testLoginMalformedJsonRaisesLakeSessionException() throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/session/login", exchange -> {
            try {
        ...
        try {
            ...
            LakeSessionException exception = Assert.expectThrows(LakeSessionException.class, handle::login);
    ...
    @Test(groups = {"UNIT"})
    public void testStreamingLoadMalformedJsonRaisesLakeStreamingLoadException() throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        ...
        try {
            ...
            LakeStreamingLoadException exception = Assert.expectThrows(LakeStreamingLoadException.class, () ->
    ...
    @Test(groups = {"UNIT"})
    public void testStreamingLoadRequestEncodingFailureRaisesLakeStreamingLoadException() throws Exception {
        HttpServer server = HttpServer.create(new InetSocketAddress(0), 0);
        server.createContext("/v1/streaming_load", exchange -> {
            try {
    ...
    public void testUploadStreamCompressionFailureRaisesSQLException() {
        LakeSessionHandle handle = createSessionHandle(URI.create("http://127.0.0.1:1"));
    ...
    public void testUploadStreamPresignedServiceUnavailableRaisesSQLExceptionWithPresignCause() throws Exception {
        ...
        uploadServer.createContext("/upload", exchange -> {
            try {
                ...
                byte[] response = "temporary".getBytes(StandardCharsets.UTF_8);
    ...
    public void testDownloadStreamPresignResponseWithoutRowsRaisesSQLExceptionWithPresignCause() throws Exception {
        ...
        try {
            LakeSessionHandle handle = createSessionHandle(URI.create("http://127.0.0.1:" + server.getAddress().getPort()));
    ...
    public void testDownloadStreamPresignQueryErrorRaisesSQLException() throws Exception {
        ...
        try {
            ...
            SQLException exception = Assert.expectThrows(SQLException.class,
                    () -> handle.downloadStream("~", "path/file.txt"));
            Assert.assertTrue(exception.getMessage().contains("Failed to prepare presigned download request"), exception.getMessage());
            Assert.assertTrue(exception.getCause() instanceof LakeQueryException, String.valueOf(exception.getCause()));
    ...
    private static LakeSessionHandle createSessionHandle(URI baseUri) {
        return new LakeSessionHandle(
                new OkHttpClient.Builder()
                        .addInterceptor(userAgentInterceptor(DriverInfo.USER_AGENT_VALUE))
                        .build(),
                SessionHandleConfig.builder()
    ...
    private static void invokeSendHeartbeat(LakeSessionHandle handle, List<QueryLiveness> queryLivenesses) throws Exception {
        Method method = LakeSessionHandle.class.getDeclaredMethod("sendHeartbeat", List.class);
        method.setAccessible(true);
        method.invoke(handle, queryLivenesses);
    }

    private static void invokeExecuteStageUpload(LakeSessionHandle handle, Request request) throws Exception {
        Method method = LakeSessionHandle.class.getDeclaredMethod("executeStageUpload", Request.class);
        method.setAccessible(true);
        try {
