...
@Test(timeOut = 10000)
public class TestLakeDatabaseMetaData {
...
//                    Assert.assertEquals(remarks, "'test comment'");
    ...
    public void testColumnsMeta() throws Exception {
        try (Connection connection = Utils.createConnection()) {
            ...
            System.out.println("====================================");
            try (ResultSet rs = connection.getMetaData().unwrap(LakeDatabaseMetaData.class).getColumns(null, "default", "test_column_meta", new String[]{"v1", "u1"})) {
                while (rs.next()) {
                    String tableSchem = rs.getString("table_schem");
                    String tableName = rs.getString("table_name");
                    String columnName = rs.getString("COLUMN_NAME");
    ...
    public void testLikeString() {
        assertNull(LakeDatabaseMetaData.stringColumnLike("col_name", null));
        assertNull(LakeDatabaseMetaData.stringColumnLike("col_name", ""));
        assertEquals(LakeDatabaseMetaData.stringColumnLike("col_name", "%"), "col_name LIKE '%'");
        assertEquals(LakeDatabaseMetaData.stringColumnLike("col_name", "test%"), "col_name LIKE 'test%'");
        assertEquals(LakeDatabaseMetaData.stringColumnLike("col_name", "test"), "col_name = 'test'");
    }
}
