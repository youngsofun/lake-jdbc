...
public class LakePreparedStatement extends LakeStatement implements PreparedStatement {
    private static final Logger logger = Logger.getLogger(LakePreparedStatement.class.getPackage().getName());
    ...
    private final LakeParameterMetaData paramMetaData;
    ...
    LakePreparedStatement(LakeConnection connection, Consumer<LakeStatement> onClose, String sql) throws SQLException {
        ...
        if(this.rawStatement.getSubStatements().size() > 1) {
            throw new SQLException("Lake do not support multi statement for now");
        }
    ...
    private StageAttachment uploadBatches() throws SQLException {
        ...
        try (FileInputStream fis = new FileInputStream(saved)) {
            ...
            // upload to stage
            c.unwrap(LakeConnection.class).uploadStream(null, stagePrefix, fis, fileName, saved.length(), false);
            String stagePath = "@~/" + stagePrefix + fileName;
            return buildStateAttachment((LakeConnection) c, stagePath);
        } catch (Exception e) {
    ...
    static StageAttachment buildStateAttachment(LakeConnection conn, String stagePath) {
     ...
     * stage in Lake.
     * A stage in Lake is a temporary storage area where data files are stored
     * before being loaded into the Lake database.
     *
     * @param conn The LakeConnection object which contains the connection
     *                   details to the Lake database.
     * @param stagePath  The path of the stage in the Lake database.
    ...
    static StageAttachment buildStateAttachment(Connection conn, String stagePath) {
        LakeConnection connection = (LakeConnection) conn;
